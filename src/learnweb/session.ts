...
export class LearnwebSession {
  ...
  /**
   * Erzeugt bzw. ersetzt die Singleton-Instanz mit explizit übergebenen
   * Credentials. Für die Web-App, die die Zugangsdaten zur Laufzeit aus der
   * macOS-Keychain liest (statt über die statischen config-Konstanten beim
   * Modul-Import). getInstance() bleibt für das CLI unverändert.
   */
  public static initWithCredentials(
    baseUrl: string,
    username: string,
    password: string
  ): LearnwebSession {
    const normalizedUrl = baseUrl.trim().replace(/\/+$/, "");
    LearnwebSession.instance = new LearnwebSession(normalizedUrl, username, password);
    return LearnwebSession.instance;
  }
  ...
  /**
   * Schreibt die aktuellen Session-Cookies als Netscape-Cookie-Datei, damit
   * externe Tools (yt-dlp/ffmpeg) dieselbe authentifizierte Moodle-Session
   * nutzen können. Stellt zuvor sicher, dass eine Session existiert.
   *
   * Warum nötig: Aufzeichnungen liegen häufig als großer HLS-/Opencast-Stream
   * auf einem anderen Host als pluginfile.php — downloadFile() (RAM-Buffer,
   * SSRF-Guard, 25-MB-Cap) ist dafür ungeeignet. yt-dlp streamt zu Disk und
   * braucht die Cookies als Datei.
   *
   * Sicherheit: Die Datei enthält das Moodle-Session-Token. Sie wird mit
   * mode 0600 geschrieben und sollte vom Caller nach Gebrauch gelöscht werden.
   * Cookie-Werte werden hier niemals geloggt.
   */
  public async exportCookieFile(targetPath: string): Promise<void> {
    await this.ensureLoggedIn();
    const cookies = await this.jar.getCookies(this.baseUrl);
    const fallbackExpiry = Math.floor(Date.now() / 1000) + 86_400;
    const lines = [
      "# Netscape HTTP Cookie File",
      "# Generated by tbmn-learnweb-connector for local yt-dlp/ffmpeg use. Do not edit.",
      "",
    ];
    for (const c of cookies) {
      const domain = c.domain ?? new URL(this.baseUrl).hostname;
      // tough-cookie: hostOnly===false bedeutet ein Domain-Cookie (gilt für Subdomains).
      const hostOnly = c.hostOnly !== false;
      const includeSubdomains = hostOnly ? "FALSE" : "TRUE";
      const netscapeDomain = hostOnly
        ? domain
        : domain.startsWith(".")
          ? domain
          : `.${domain}`;
      const secure = c.secure ? "TRUE" : "FALSE";
      // Session-Cookies (expires === "Infinity") haben kein Date — wir setzen einen
      // Zeitpunkt in der Zukunft, damit yt-dlp sie nicht als abgelaufen verwirft.
      const expiry =
        c.expires instanceof Date ? Math.floor(c.expires.getTime() / 1000) : fallbackExpiry;
      // curl/yt-dlp-Konvention: httpOnly-Cookies bekommen das #HttpOnly_-Präfix.
      const prefix = c.httpOnly ? "#HttpOnly_" : "";
      lines.push(
        `${prefix}${netscapeDomain}\t${includeSubdomains}\t${c.path || "/"}\t${secure}\t${expiry}\t${c.key}\t${c.value}`
      );
    }
